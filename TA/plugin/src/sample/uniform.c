...
void cfe_uniform_sample(mpz_t res, mpz_t upper) {
    ...
    size_t n_bits = 0;
    size_t n_bytes = 0;
    uint8_t *rand_bytes = NULL;

    n_bits = mpz_sizeinbase(upper, 2);
    n_bytes = ((n_bits - 1) / 8) + 1;
    rand_bytes = (uint8_t *) cfe_malloc(n_bytes * sizeof(uint8_t));

    while (1) {
        /* #OPTEE */
        /* randombytes_buf(rand_bytes, n_bytes); */ // get random bytes
        TEE_GenerateRandom(rand_bytes, n_bytes); // get random bytes
...
void cfe_uniform_sample_vec_det(cfe_vec *res, mpz_t max, unsigned char *key) {
    size_t n_bits_max = 0;
    size_t n_bytes_for_vec = 0;
    mpz_t num, el;
    uint8_t *rand_bytes = NULL;
    size_t j = 0, k = 0;

    n_bits_max = mpz_sizeinbase(max, 2);
    n_bytes_for_vec = ((n_bits_max * res->size - 1) / 8) + 1;

    mpz_inits(num, el, NULL);

    for (size_t i = 3;; i++) {
        rand_bytes = (uint8_t *) cfe_malloc(i * n_bytes_for_vec * sizeof(uint8_t));
